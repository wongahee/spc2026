...
def delete_document(source):
    # 1. MongoDB처럼 NoSQL 구문 작성 -> 내 DB에서 해당 소스를 갖는 청크 삭제
    store._collection.delete(where={"source": source})

    # 2. DATA 안에 pdf 파일 보관 중이라면 삭제
...
@app.get('/files')
def files():
    # Vector DB(ChromaDB) 안에 있는 파일 목록 가져오기
    # 파일 목록이 따로 저장되어있지 않아, 모든 청크 데이터를 읽고 그 안에 있는 src/chunks 갯수를 세어 문서로 간주함
