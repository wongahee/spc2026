...
summarize_chain = (
    ChatPromptTemplate.from_messages([
        ("system", "너는 뉴스 요약 전문가이다."),
        ("human", "다음 뉴스를 이미지로 생성하기 좋게 요약해라.")
    ])
    | llm
    | StrOutputParser()
)
...
pipeline = (
    ...
    | RunnablePassthrough.assign(image_pate=generate_image)
)

def main():
    result = pipeline.invoke({"query": "젠슨황 4박 5일 한국 방문 일정"})
