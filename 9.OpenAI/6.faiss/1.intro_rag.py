...
def rag_query(user_query):
    ...
    _, indices = index.search(np.array([query_embedding]), k=1)     # 벡터 DB에서 질문 값 (user_query)의 숫자값과 가까운 값 갯수 설정(k)
    retrieved_doc = documents[indices[0][0]]

    prompt = f"""아래 내용을 보고 답변하시오. 아래 질문에 관련 자료가 연관이 없거나 답변할 수 없는 내용이면, 모른다고 답변하시오. 그리고 적절한 이모티콘을 사용하시오
